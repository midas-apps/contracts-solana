...
export const getMultisigInfo = async <T extends boolean = true>(
  connection: Connection,
  multisigPda: PublicKey,
  required?: T,
): Promise<T extends true ? multisig.accounts.Multisig : multisig.accounts.Multisig | null> => {
  required ??= true as T;
  return await multisig.accounts.Multisig.fromAccountAddress(connection as any, multisigPda).catch(
    (err) => {
      if (required) {
        throw err;
      }
      ...
      if (
        (err instanceof Error && err.message.includes('Unable to find Multisig account at')) ||
        err.message.includes('Expected  to hold a COption')
      ) {
        return null;
      }
...
export const wrapTxWithSquadsSigner = async (
  connection: Connection,
  {
    instructions,
    multisigSignerPda,
  ...
  const [messageDecoded] = multisig.types.transactionMessageBeet.deserialize(
...
export const sendTxWithTimelock = async (
  connection: Connection,
  {
    instructions,
    timelock,
  ...
  if (multisigInfo.members.length !== 1) {
    throw new Error(
      'Expected timelock to have only one member, got ' + multisigInfo.members.length,
    );
  }
  ...
  if (squadsSigner) {
    console.log(
      'Timelock member is a squads signer, timelock txs will be wrapped with squads signer',
    );
  }
  ...
  if (action.type === 'create') {
    ...
    const createdProposal: multisig.accounts.Proposal | null =
      await multisig.accounts.Proposal.fromAccountAddress(connection as any, proposalPda).catch(
        (err) => {
    ...
    if (createdProposal.status.__kind !== 'Approved') {
      throw new Error(
        'Expected proposal to be approved, but the status is ' + createdProposal.status.__kind,
      );
    } else {
...
export const getTimelockTransaction = async (
  connection: Connection,
  {
    timelock,
    transactionIndex,
  ...
  const transaction: multisig.accounts.VaultTransaction | null =
    await multisig.accounts.VaultTransaction.fromAccountAddress(
      ...
      if (
        ...
        err.message.includes('Unable to find VaultTransaction account at')
      ) {
        return null;
      }
      throw err;
  ...
  const proposal: multisig.accounts.Proposal | null =
    await multisig.accounts.Proposal.fromAccountAddress(connection as any, proposalPda).catch(
      (err) => {
        if (err instanceof Error && err.message.includes('Unable to find Proposal account at')) {
          return null;
...
export const deployTimelock = async (
  common: CommonParams,
  { delay, member, createKey }: DeployTimelockConfig,
) => {
