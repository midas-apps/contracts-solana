...
async function main(provider: AnchorProvider, payer: Wallet, network: string) {
  ...
  if (!transaction || !proposal) {
    throw createUserError(
      `Transaction or proposal not found for multisig tx index ${multisigTxIndex}. Its either not created or already executed`,
    );
  }
  ...
  if (
    BigInt(proposal.status.timestamp.toString()) + BigInt(multisigInfo.timeLock) >
    BigInt(currentTs)
  ) {
  ...
  const program = Object.keys(programAddresses).find((key) =>
    programAddresses[key as keyof typeof programAddresses].equals(programId),
  );
  ...
  if (!currentAuthority.equals(existingTimelock.vault)) {
    throw createUserError(
      `Current authority is not the timelock vault, it's ${currentAuthority.toBase58()}`,
    );
  }
