...
async function main(provider: AnchorProvider, payer: Wallet, network: string) {
  ...
  console.log(
    `Transferring upgrade authority for program ${program} to ${newAuthority.toBase58()} on network ${network}`,
  );
  ...
  if (!data) {
    throw createUserError(
      `No program data found for ${program} at address ${programDataPda.toBase58()}`,
    );
  }
  ...
  if (!newAuthority.equals(timelock.vault) && !newAuthority.equals(payer.publicKey)) {
    throw createUserError(
      `New authority is not the timelock vault or the payer, it's ${newAuthority.toBase58()}`,
    );
  }
  ...
  const { tx, newTransactionIndex } = currentAuthority.equals(timelock.vault)
    ? await sendTxWithTimelock(common.provider.connection, {
        instructions: [inx],
