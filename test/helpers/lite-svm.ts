...
export class LiteSVMProvider implements Provider {
  ...
  constructor(
    ...
    this.connection = new LiteSVMConnectionProxy(client) as unknown as Connection; // uh
...
export function fromWorkspace(
  ...
  const programs = (parsedToml['programs'] as TOML.JsonMap)['localnet'] as TOML.JsonMap;
  ...
  additionalPrograms.forEach((program) => {
    ...
    const programPath = path.join(`${program.name.replace('-', '_')}.so`);
    svm.addProgramFromFile(id, programPath);
  });

  prefundedAccounts.forEach((account) => {
    svm.airdrop(account.publicKey, 1000n * BigInt(LAMPORTS_PER_SOL));
  });
