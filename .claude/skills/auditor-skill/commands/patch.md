...
A **cosmetic patch ≠ FIXED**. Never claim `[FIX-VERIFIED]` without an _executed_ revert of the finding's own exploit; a patch that reads plausibly but was never run against the PoC is `[FIX-PROPOSED]` at most. The patch is proposal-grade evidence for the client to apply and re-test on their side — this command does not touch the client's real tree.
