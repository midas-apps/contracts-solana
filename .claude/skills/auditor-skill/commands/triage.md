...
1. **Dedup by root-cause signature.** Collapse candidates that share a **root-cause signature** — the normalized code shape of the bug (e.g. a missing signer check, an unchecked `*`, a PDA derived without the stored bump), the same signature `/auditor:re-audit` uses for its sibling sweep. Two candidates with the same signature at the same root cause are **one** finding (record all locations); the same signature at _different_ sites are siblings (keep separate, note the shared class).
