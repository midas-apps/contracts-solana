...
>
> _(Adapted from safe-solana-builder `references/native-rust.md`.)_
...
_before any field of that account's data is read or trusted_:
...
- **Discriminator before field use** stops a same-shape account of a _different type_ (both
...
deserialized _before_ the CPI is stale. To use the account's state after the CPI you must
