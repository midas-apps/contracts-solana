...
>
> **Purpose:** This is a _dev-time_ reference, not a per-audit checklist. Build failures are
...
>
> _(Adapted from safe-solana-builder `references/{anchor,native-rust,pinocchio}.md` §8 / §10.)_
...
instructions; that part _is_ a check, tracked in `pinocchio.md`.)
...
- [ ] **`overflow-checks = true`** in `[profile.release]` — else runtime arithmetic checks are inert (§1.6) — _this one is audit-relevant, flag it if absent_
