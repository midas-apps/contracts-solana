...
reference decides _which_ harness reproduces it and _what tier_ the result earns.
...
**One rule above all:** _executable evidence outranks prose, but prose is never
removed._ An unbuildable or un-forkable target still yields a complete finding — you
...
_blocker to name in the report_, not a reason to skip the finding or fake a result.
...
Pick by what the bug actually _is_. The cheapest harness that faithfully reproduces
...
| **parse / deser / math** that is **fuzz-discoverable** (the crashing input is not obvious)                                      | **Trident stateful** (default) or **cargo-fuzz unit** (pure function) | let the fuzzer _find_ the input; emit the crashing artifact as the proof                                | `[PoC-FUZZ-REPRODUCED]`                                                   |
...
A cosmetic patch ≠ FIXED. `[FIX-VERIFIED]` requires an _executed_ revert of the
