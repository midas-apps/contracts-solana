...
> oracle-anchored swap). Oracle handling is _per-call-site_, not per-instruction — run
...
A `PriceUpdateV2` (or `PullFeed`, or legacy `PriceAccount`) that _deserializes_ is not a
...
- PASS: `get_price_no_older_than(...)`, or `get_price_unchecked` _followed by_ an explicit
...
1–3% = 100–300 bps). A wide band during a crash or thin-market moment is _the only_ signal
...
- FAIL: `conf` read but never compared; or compared for _presence_ (`conf != 0`) rather
...
- **Freshness** applies to the _staler_ leg: `max(age_a, age_b) <= MAX_AGE`. Each
  `get_price_no_older_than` enforces its own leg; confirm _both_ are gated (a fresh SOL
...
But a program that accepts the clock as an `AccountInfo`/`Sysvar` _parameter_ must verify
...
A protocol that accepts _another protocol's_ token (LP token, LST, vault share) as
...
- **Inherited oracle risk:** if protocol B's NAV/price feed is weak (fails §1–§11 under B's
  ...
  question is: what oracle does the issuer use, and does _it_ pass this file?
