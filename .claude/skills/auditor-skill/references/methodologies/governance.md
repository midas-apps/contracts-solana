...
>
> **Public exploit provenance:** most catastrophic governance incidents are _configuration_
...
timestamp**. The timelock is only as strong as the _shortest_ per-transaction hold-up. A
...
- **NFT collection spoofing:** for NFT-gated weight, the NFT's **collection** must be verified
  (`verified == true` and the expected collection key), not just that the account is _an_ NFT.
...
- FAIL: proposals reviewed by title/description only; a mismatch between prose and encoded
  instruction goes undetected. This is a _review-process_ finding as much as a code one.
...
indices against the ALT _at execution time_. If the ALT can be mutated (extended / entries
