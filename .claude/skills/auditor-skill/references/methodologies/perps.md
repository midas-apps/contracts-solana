...
> covers the base case the note says _"beyond `<ID>`, also verify…"_.
>
...
_Vulnerable → fixed (the Mango class):_
...
- **Settlement ordering (the Offside/RateX class).** Funding settled _before_ liquidation can let an
...
_Funding symmetry invariant:_ total long-side debit == total short-side credit — the protocol must not
...
- **Withdraw leaves an IM buffer.** `withdraw_collateral` must leave post-withdraw health ≥ **IM**
  (not merely MM), and be bounded by _free_ collateral, not total.
...
- **Reward after health check** — computing the liquidator reward _before_ the health check enables a
...
transactions**: the user submits a _request_ (open/increase/decrease/close), then a **keeper/crank**
executes it against a price fetched at execution time. That gap between submit and execute is a rich
surface — the user can react to price _after_ requesting, and the keeper is a privileged actor who
...
_Public exploits & reports referenced: Mango Markets (2022, $115M — oracle composition + no position
...
theft). Funding-rate, margin, and mark/index mechanics are public derivatives math._
