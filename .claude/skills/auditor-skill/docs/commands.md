...
Verification-verb discipline: use "we re-tested and confirmed" only when _you_ verified against the current tree; a client's "we fixed it" stays "client states fixed (unverified)" and is not FIXED until independently confirmed.
