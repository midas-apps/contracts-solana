...
A **rejected** finding looks like: _"input ≥ 16 and header = 8 ⟹ input − header ≥ 8, so the subtraction cannot underflow. Downgraded `[FAIL-7]` → `[UNCONFIRMED]`."_
...
| `[N/A]`     | _why_ it doesn't apply. Never bare N/A.                                          |
