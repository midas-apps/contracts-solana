...
4. **`peer-reviewer`** (opus) — independent reconciliation on top-severity findings (N≥8, contested N≥7): re-derives from the code (reusing the context worksheets), delegates to Trail of Bits `second-opinion`/`fp-check` when present; emits CONFIRM / DISPUTE / DOWNGRADE. _(Neodyme independent dual-review.)_
