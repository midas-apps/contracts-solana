...
Pool-vs-trader and RFQ perps (Jupiter Perps, GMX-Solana, Adrena) — and many vault/order programs — split an action into **two transactions**: the user submits a _request_ PDA (open / increase / decrease / close), and a separate **keeper/crank** later _executes_ it against a price fetched at execution time. The gap between submit and execute is the vulnerability. Two actors can abuse it:
