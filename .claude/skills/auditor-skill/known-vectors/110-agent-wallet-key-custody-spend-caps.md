...
This vector targets the CUSTODY and LIMITS around an agent's signing key. It is distinct from blind signing (see 113, which is about _what_ gets signed): even a perfectly-decoded transaction is catastrophic if the key is exfiltratable or the per-tx/per-epoch value is unbounded.
