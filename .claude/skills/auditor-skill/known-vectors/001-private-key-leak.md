...
**Step 8: Check NEXT*PUBLIC* vars for secret leakage**

```
grep -rn "NEXT_PUBLIC_" --include="*.ts" --include="*.tsx" --include="*.env*" . | grep -v node_modules
```

- ✅ PASS: All NEXT*PUBLIC* vars are truly public (RPC URL without key, program ID, network name)
- ❌ FAIL: Any NEXT*PUBLIC* var containing a secret key, API secret, or private key
