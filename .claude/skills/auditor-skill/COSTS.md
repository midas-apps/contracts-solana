...
> Token figures below use the **~4 chars/token** rule of thumb (measured against the on-disk corpus). "Fixed" means _always loaded, every audit regardless of repo_. Scope-gating and trigger-gating (Rule 0 + SKILL Scope-Gated Loading) moved the checklists, known-vectors, and references out of the fixed floor: only the in-scope subset is ever read.
